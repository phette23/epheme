...
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install git mongodb cron python python-pip python-dev build-essential python-setuptools

RUN pip install virtualenv
RUN git clone https://github.com/phette23/epheme /epheme
RUN cd /epheme && pip install -r requirements.txt
